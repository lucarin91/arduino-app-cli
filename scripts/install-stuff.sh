...
# fix TMPDIR
adb shell tee /etc/profile.d/50-fix-adb-shell.sh > /dev/null << EOF
export TMPDIR=/tmp
EOF

# install repo
adb shell tee /etc/apt/sources.list.d/arduino.list > /dev/null << EOF
deb [signed-by=/etc/apt/keyrings/arduino.asc] https://apt-repo.arduino.cc stable main
EOF

adb shell tee /etc/apt/sources.list.d/arduino-unstable.list > /dev/null << EOF
deb [signed-by=/etc/apt/keyrings/arduino.asc] https://apt-repo.arduino.cc unstable main
EOF


adb shell tee /etc/apt/keyrings/arduino.asc > /dev/null << EOF
...
adb shell apt-get --fix-broken install
adb shell apt-get install arduino-cli --yes
adb shell apt-get install arduino-router --yes
